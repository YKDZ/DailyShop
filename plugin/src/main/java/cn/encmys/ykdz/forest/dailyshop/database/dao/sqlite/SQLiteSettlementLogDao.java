...
import cn.encmys.ykdz.forest.dailyshop.api.database.dao.SettlementLogDao;
...
public class SQLiteSettlementLogDao implements SettlementLogDao {
    ...
    @Override
    public void insertLog(SettlementLog log) {
        try (Connection conn = DailyShop.DATABASE_FACTORY.getConnection();) {
            ...
            pStmt.setString(5, gson.toJson(log.getOrderedProducts(), new TypeToken<Map<String, Integer>>() {}.getType()));
            pStmt.setString(6, log.getSettledShop());
            pStmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
