...
public class TextUtils {
    ...
    @NotNull
    public static List<Component> decorateTextToComponent(@Nullable List<String> text, @Nullable Player player, @Nullable Map<String, Object> normalVars, @Nullable Map<String, List<Object>> listVars) {
    ...
    @Nullable
    public static Component decorateTextToComponent(@Nullable String text, @Nullable Player player, @Nullable Map<String, Object> normalVars) {
        if (text == null) return null;

        return HyphaAdventureUtils.getComponentFromMiniMessage(decorateText(text, player, normalVars));
    }

    public static List<String> decorateText(@NotNull List<String> text, Player player, @Nullable Map<String, Object> normalVars, @Nullable Map<String, List<Object>> listVars) {
        return parsePlaceholder(parseInternalVar(parseInternalListVar(text, listVars), normalVars), player);
    }

    public static String decorateText(@Nullable String text, @Nullable CommandSender sender, @Nullable Map<String, Object> normalVars) {
        if (sender instanceof Player) return decorateText(text, (Player) sender, normalVars);
        else return decorateText(text, null, normalVars);
    }

    public static String decorateText(@Nullable String text, @Nullable Player player, @Nullable Map<String, Object> normalVars) {
        if (text == null) {
            return null;
        }
    ...
    public static List<String> parseInternalVar(@NotNull List<String> text, @Nullable Map<String, Object> vars) {
    ...
    @Nullable
    public static String parseInternalVar(@NotNull String line, @Nullable Map<String, Object> vars) {
        if (vars == null) {
            return line;
        }
        // 正则表达式匹配所有 {} 包裹的变量
        ...
        while (matcher.find()) {
            ...
            String value = toString(vars.get(key));
            // 变量的值是否为 "-1"、null 或空字符串
            // 如果是，则可能触发 optionalMarker 机制
            if (value.equals("-1") || value.isBlank()) {
                hasNullValue = true;
            }
            // 将前面的内容添加到 result
    ...
    public static List<String> parseInternalListVar(@NotNull List<String> lines, @Nullable Map<String, List<Object>> vars) {
        if (vars == null) {
            return lines;
        }
        List<String> result = new ArrayList<>();
        Set<Map.Entry<String, List<Object>>> entries = vars.entrySet();
    ...
    private static @NotNull String toString(@Nullable Object object) {
        if (object == null) {
            return "";
